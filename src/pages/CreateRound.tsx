    ...
    >
      ...
      {step === 'info' && (
        <div className="flex flex-col gap-4">
          ...
          <div className="bg-gold-500/10 border border-gold-500/25 rounded-xl p-4 text-sm text-gold-400">
          ...
          {teams.map((team, ti) => {
            ...
            return (
              <div key={team.id} className="card overflow-visible">
                {/* Team header */}
                <div className="px-4 py-3 flex items-center gap-3 border-b border-gold-500/15">
                  ...
                  <input
                    ...
                    className="flex-1 text-base font-semibold text-cream bg-transparent
                               focus:outline-none placeholder-cream/30"
                  />
                  {/* Color picker */}
                    ...
                    {TEAM_COLORS.map((c) => (
                      <button
                        ...
                        className={`w-6 h-6 rounded-full ${c.dot} transition-transform active:scale-90 ${
                          team.color === c.value ? 'ring-2 ring-offset-1 ring-gold-400 ring-offset-forest-800' : ''
                        }`}
                        aria-label={c.label}
                      />
                    ))}
                ...
                <div className="p-4 flex flex-col gap-2.5">
                  <p className="text-xs font-medium text-cream/40 uppercase tracking-wide">
                    Players
                  </p>
                  {team.players.map((player, pi) => (
                    <input
                      ...
                      className="w-full bg-forest-950 border border-gold-500/20 rounded-lg px-3 py-2.5
                                 text-sm text-cream placeholder-cream/30 focus:outline-none
                                 focus:border-gold-500/60 transition-colors"
                    />
                  ))}
                </div>
                ...
                {ti === 0 && (
                  <div className="px-4 pb-3 -mt-1">
                    <p className="text-xs text-gold-400/80 bg-gold-500/10 rounded-lg px-3 py-2">
          ...
          <div className="card p-4">
            <div className="flex justify-between text-sm">
              <div className="text-center">
                <div className="text-xs text-cream/40 mb-1">Front 9</div>
                <div className="text-lg font-bold text-cream">{front9Par}</div>
              </div>
              <div className="text-center">
                <div className="text-xs text-cream/40 mb-1">Back 9</div>
                <div className="text-lg font-bold text-cream">{back9Par}</div>
              </div>
              <div className="text-center">
                <div className="text-xs text-cream/40 mb-1">Total Par</div>
                <div className="text-xl font-bold text-gold-400">{front9Par + back9Par}</div>
              </div>
            </div>
          </div>
          ...
          {['Front 9', 'Back 9'].map((nine, nineIdx) => (
            <div key={nine} className="card">
              <div className="px-4 py-3 border-b border-gold-500/15">
                <p className="text-sm font-semibold text-cream/70">{nine}</p>
              </div>
                ...
                {pars.slice(nineIdx * 9, nineIdx * 9 + 9).map((par, j) => {
                  ...
                  return (
                    <div key={holeIdx} className="flex flex-col items-center gap-1">
                      <span className="text-xs text-cream/40">Hole {holeIdx + 1}</span>
                          ...
                          <button
                            ...
                            className={`w-9 h-9 rounded-lg text-sm font-semibold transition-all active:scale-95 ${
                              par === p
                                ...
                                : 'bg-forest-950 text-cream/60 border border-gold-500/20'
                            }`}
                          >
                            {p}
                          </button>
